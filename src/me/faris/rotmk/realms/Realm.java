    ...
    private static Realm addRealm(Realm realm) {
        try {
            for (Realm existingRealm : REALM_LIST) {
                if (existingRealm.getID() == realm.getID())
                    throw new DuplicateRealmException("The Realm with the ID " + realm.getID() + " already exists!", realm.getID());
            }
            if (!REALM_LIST.contains(realm)) REALM_LIST.add(realm);
            else
                throw new DuplicateRealmException("The Realm '" + realm.getName() + "' has already been registered!", realm.getID());
            return realm;
