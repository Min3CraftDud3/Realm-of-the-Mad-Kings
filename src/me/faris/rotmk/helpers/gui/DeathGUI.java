...
public class DeathGUI implements Listener {
	...
	public void openInventory() {
		if (this.player != null && !deathPlayers.containsKey(this.player.getName())) {
			if (this.deathInventory == null) this.deathInventory = this.player.getServer().createInventory(this.player, 9, ChatColor.RED + this.player.getName() + ", you died!");
			...
			if (event.getWhoClicked() instanceof Player) {
				...
				if (event.getInventory().getHolder() instanceof Player) {
					if (event.getInventory().getName().equals(this.deathInventory.getName())) {
						if (!player.getName().equals(((Player) event.getInventory().getHolder()).getName())) player.sendMessage(ChatColor.RED + "You're not the holder of this inventory.");
