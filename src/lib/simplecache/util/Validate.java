...
public class Validate {
	 ...
	 * @param classFile  the <code>Class</code> which the collection's elements are expected to be, not null
	 * @param message  the exception message if the <code>Collection</code> has elements not of type <code>clazz</code>
	 * @since 2.1
	 */
	public static void allElementsOfType(Collection<?> collection, Class<?> classFile, String message) {
		Validate.notNull(collection);
		Validate.notNull(classFile);
		for (Iterator<?> it = collection.iterator(); it.hasNext();) {
			if (classFile.isInstance(it.next()) == false) {
				throw new IllegalArgumentException(message);
			}
		}
	}
